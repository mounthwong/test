    ...
    var Controller = {
        index: function () {
            ...
            setInterval(function(){
                $(".notifications-menu ul.menu").empty();
                $.ajax({
                    url: 'order/order/getUnreadOrderList',
                    type: 'post',
                    dataType: 'jsonp',
                    success: function (ret) {
                        ...
                        $.each(ret.newslist, function (i, j) {
                            var item = '<li><a href="javascript:void(0);" target="_blank"><i class="' + j.icon + '"></i> ' + j.truename+"&nbsp;&nbsp"+j.phone+"&nbsp;&nbsp;"+timestampToTime(j.createtime)+ '</a></li>';
                            $(item).appendTo($(".notifications-menu ul.menu"));
                        });
                        if(ret.new>0){
                            var audio = new Audio();
                            audio.src ='http://www.hnumg.cn/managers/public/order.mp3';
                            audio.play();
                        }
                    }
                });
